...
function migrate(data) {
  if (data.users && data.memberships) return data;
...
function teacherClasses() {
  if (isAdmin()) return state.classes;
  return state.classes.filter((item) => item.teacherId === user().id);
}

function studentClasses() {
  const ids = state.memberships.filter((item) => item.userId === user().id).map((item) => item.classId);
  return state.classes.filter((item) => ids.includes(item.id));
}
...
function myActivities() {
  ...
  return state.activities.filter((item) => ids.includes(item.classId));
}
...
function navItems() {
  if (isAdmin()) return ["dashboard", "users", "classes", "activities", "exams", "records"];
  return canTeach()
...
function adminDashboardView() {
  ...
  return `
    <section class="hero">
      ...
      <div class="quick-stack">
        ${quickAction("Review users", "users")}
        ${quickAction("Review classes", "classes")}
        ${quickAction("Review records", "records")}
      </div>
    </section>
    <section class="grid">
...
async function saveSubmission(event) {
  ...
  const checked = activity.mode === "auto" ? checkAnswers(activity, answers) : { score: null, maxScore: totalPoints(activity), feedback: "" };
  ...
  const submission = {
    ...
    checkedAt: activity.mode === "auto" ? new Date().toISOString().slice(0, 10) : null
  };
  if (cloudMode) {
  ...
  persist();
  render();
  toast(activity.mode === "auto" ? `Auto-checked: ${checked.score}/${checked.maxScore}` : "Submission saved.");
}
...
function saveUser(event) {
  ...
  const data = Object.fromEntries(new FormData(event.target));
  if (state.users.some((item) => item.email.toLowerCase() === data.email.toLowerCase())) return toast("Email already exists.");
  state.users.push({ id: crypto.randomUUID(), name: data.name, email: data.email, password: data.password, role: data.role, school: data.school });
  persist();
  render();
...
async function saveSchool(event) {
  event.preventDefault();
  if (!cloudMode || !isAdmin()) return toast("Super admin cloud mode is required.");
  const data = Object.fromEntries(new FormData(event.target));
...
async function saveAdminInvite(event) {
  event.preventDefault();
  if (!cloudMode || role() !== "super_admin") return toast("Only super admin can invite admins.");
  const data = Object.fromEntries(new FormData(event.target));
...
async function saveAdminUser(targetUserId, forcedStatus = null) {
  if (!cloudMode || role() !== "super_admin") return toast("Only super admin can manage users.");
...
function quickAction(label, view) {
  return `<button class="quick" data-view="${view}"><b>${label}</b><span>Next</span></button>`;
}
